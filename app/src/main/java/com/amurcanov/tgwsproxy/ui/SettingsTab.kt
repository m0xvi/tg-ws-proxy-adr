...
fun SettingsTab(settingsStore: SettingsStore) {
    ...
    val savedPrivateRelayToken by settingsStore.privateRelayToken.collectAsStateWithLifecycle(initialValue = "")
    ...
    var privateRelayEnabled by rememberSaveable(savedPrivateRelayEnabled) { mutableStateOf(savedPrivateRelayEnabled) }
    var privateRelayDomain by rememberSaveable(savedPrivateRelayDomain) { mutableStateOf(savedPrivateRelayDomain) }
    var privateRelayToken by rememberSaveable(savedPrivateRelayToken) { mutableStateOf(savedPrivateRelayToken) }
    ...
    fun scheduleSave() {
        ...
        saveJob = scope.launch {
            ...
            settingsStore.saveAll(
                ...
                privateRelayEnabled, privateRelayDomain, privateRelayToken,
                secretKeyText
            )
        }
    }
        ...
        AppSectionCard {
            Column(
                ...
                OutlinedButton(
                    ...
                    enabled = !cfEnabled && !privateRelayEnabled && !isRunning,
                    modifier = Modifier.fillMaxWidth().height(48.dp),
                    shape = RoundedCornerShape(24.dp),
                    ...
                    border = BorderStroke(1.dp, MaterialTheme.colorScheme.outline.copy(alpha = if (cfEnabled || privateRelayEnabled || isRunning) 0.2f else 0.5f))
                ) {
                    Icon(Icons.Default.Settings, null, Modifier.size(20.dp))
                    if (cfEnabled || privateRelayEnabled) {
                        Spacer(Modifier.width(8.dp))
                        Text(
                            if (privateRelayEnabled) stringResource(com.amurcanov.tgwsproxy.R.string.auto_private_relay_enabled)
                            else stringResource(com.amurcanov.tgwsproxy.R.string.auto_cf_enabled),
                            fontWeight = FontWeight.SemiBold
                        )
                    } else {
                        Spacer(Modifier.width(8.dp))
            ...
            Column(
                ...
                Row(
                    ...
                    Row(
                        ...
                        Column {
                            Text(
                                stringResource(com.amurcanov.tgwsproxy.R.string.private_relay),
                                style = MaterialTheme.typography.titleSmall,
                                color = MaterialTheme.colorScheme.primary,
                                fontWeight = FontWeight.SemiBold
                            )
                            Text(
                                stringResource(com.amurcanov.tgwsproxy.R.string.private_relay_subtitle),
                                style = MaterialTheme.typography.bodySmall,
                    ...
                    Switch(
                        checked = privateRelayEnabled,
                        onCheckedChange = { enabled ->
                            privateRelayEnabled = enabled
                            if (enabled) {
                                cfEnabled = false
                                isDcAuto = true
                            }
                ...
                AnimatedVisibility(visible = privateRelayEnabled) {
                    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        OutlinedTextField(
                            value = privateRelayDomain,
                            onValueChange = {
                                privateRelayDomain = it.trim()
                                scheduleSave()
                            },
                            label = { Text(stringResource(com.amurcanov.tgwsproxy.R.string.private_relay_domain)) },
                            supportingText = { Text(stringResource(com.amurcanov.tgwsproxy.R.string.private_relay_domain_hint)) },
                            singleLine = true,
                            enabled = !isRunning,
                        ...
                        OutlinedTextField(
                            value = privateRelayToken,
                            onValueChange = { value ->
                                privateRelayToken = value.filter {
                                    it.isLetterOrDigit() || it == '-' || it == '.' || it == '_' || it == '~'
                                }.take(256)
                                scheduleSave()
                            },
                            label = { Text(stringResource(com.amurcanov.tgwsproxy.R.string.private_relay_token)) },
                            supportingText = { Text(stringResource(com.amurcanov.tgwsproxy.R.string.private_relay_token_hint)) },
            ...
            Row(
                ...
                Switch(
                    ...
                    enabled = !isRunning && !privateRelayEnabled
                )
            }
