...
// Private WSS relay. Unlike CfProxy, the relay terminates WebSocket on a
// user-controlled VPS and forwards raw MTProto bytes to the selected DC.
...
pub static PRIVATE_RELAY: Lazy<RwLock<PrivateRelayConfig>> = Lazy::new(|| {
    RwLock::new(PrivateRelayConfig {
        enabled: false,
        domain: String::new(),
        token: String::new(),
    })
...
pub struct Stats {
    ...
    pub connections_private_relay: AtomicI64,
...
impl Stats {
    pub fn summary(&self) -> String {
        ...
        format!(
            "total={} active={} ws={} tcp_fb={} cf={} relay={} bad={} err={} pool={}/{} up={} down={}",
            ...
            self.connections_private_relay.load(Ordering::Relaxed),
    ...
    pub fn summary_ru(&self) -> String {
        ...
        let relay = self.connections_private_relay.load(Ordering::Relaxed);
        if relay > 0 {
    ...
    pub fn reset(&self) {
        ...
        self.connections_private_relay.store(0, Ordering::Relaxed);
