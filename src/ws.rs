...
// Private relay credentials and MTProto traffic must never be sent through a
// TLS connection with certificate verification disabled. The relay transport
// therefore uses a separate, verified WebPKI configuration.
static VERIFIED_TLS_CONFIG: Lazy<Arc<ClientConfig>> = Lazy::new(|| {
...
// "relay.example.ru" -> ("relay.example.ru", 443)
// "relay.example.ru:8443" -> ("relay.example.ru", 8443)
// IPv6-литералы не используются (адреса DC и домены relay — v4/имена).
...
pub async fn ws_connect_once(
    ...
    ws_connect_once_with_tls(dial_addr, domain, path, timeout, TLS_CONFIG.clone()).await
}

pub async fn ws_connect_once_verified(
    dial_addr: &str,
    domain: &str,
    path: &str,
    timeout: Duration,
) -> Result<RawWebSocket, WsError> {
    ws_connect_once_with_tls(
        dial_addr,
        domain,
        path,
        timeout,
        VERIFIED_TLS_CONFIG.clone(),
    )
    .await
}

async fn ws_connect_once_with_tls(
    dial_addr: &str,
    domain: &str,
    path: &str,
    timeout: Duration,
    ...
    let (dial_host, dial_port) = split_host_port(dial_addr);
    let target_addr = format!("{}:{}", dial_host, dial_port);
    ...
    let (sni_host, _) = split_host_port(domain);
    let sni = server_name(sni_host);
