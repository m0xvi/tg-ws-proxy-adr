...
/// Configure the user-controlled WSS relay. The token is restricted to URL
/// unreserved characters because it is sent as an encrypted query parameter.
///
/// # Safety
/// String pointers must be valid C strings or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn SetPrivateRelayConfig(
    enabled: c_int,
    c_domain: *const c_char,
    c_token: *const c_char,
) {
    let domain_raw = cstr_to_string(c_domain).trim().to_lowercase();
    let token = cstr_to_string(c_token).trim().to_string();

    // Multiple private relays are supported using the same token, separated by
    // comma, semicolon or whitespace: "mg.example.ru, relay2.example.ru".
    let domains: Vec<String> = domain_raw
        .split(|c: char| c == ',' || c == ';' || c.is_whitespace())
        .map(|d| d.trim().to_string())
        .filter(|d| !d.is_empty())
        .filter(|d| {
            // "relay.example.ru" или "relay.example.ru:8443"
        ...
        .collect();
    let valid_domain = !domains.is_empty() && domains.len() <= 8;
    let normalized_domains = domains.join(",");

    let valid_token = (32..=256).contains(&token.len())
        && token
            .chars()
            .all(|c| c.is_ascii_alphanumeric() || matches!(c, '-' | '.' | '_' | '~'));

    let mut cfg = PRIVATE_RELAY.write();
    cfg.enabled = enabled != 0 && valid_domain && valid_token;
    cfg.domain = if valid_domain { normalized_domains } else { String::new() };
    cfg.token = if valid_token { token } else { String::new() };
}
