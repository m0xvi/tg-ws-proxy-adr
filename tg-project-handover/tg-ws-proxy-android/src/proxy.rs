...
async fn private_relay_acquire_ws(dc: i32) -> Option<(RawWebSocket, String)> {
    let (enabled, domain, token) = {
        let cfg = PRIVATE_RELAY.read();
        (cfg.enabled, cfg.domain.clone(), cfg.token.clone())
    };
    if !enabled || domain.is_empty() || token.is_empty() {
        return None;
    }

    // Private Relay must preserve the DC requested by Telegram. Public channel
    // media, reactions and custom emoji may use CDN/media routes such as DC203;
    // mapping DC203 to DC2 breaks those flows even when text messages work.
    let relay_dc = dc;
    let path = format!("/apiws?dc={}&token={}", relay_dc, token);
    ...
    for relay_domain in domains {
        match ws_connect_once_verified(
            &relay_domain,
            &relay_domain,
            &path,
        ...
        .await
        {
            Ok(ws) => return Some((ws, relay_domain)),
            Err(err) => {
                lwarn!(" Private relay {} DC{}: {}", relay_domain, dc, err.compact());
            }
        }
    }
    None
...
pub async fn do_fallback(
    ...
    // A private relay is always attempted first. It is the only route expected
    // to work when direct Telegram TCP is filtered and Cloudflare TLS is stalled.
    if let Some((ws, relay_domain)) = private_relay_acquire_ws(dc).await {
        if ws.send(relay_init).await.is_ok() {
            STATS
                .connections_private_relay
                .fetch_add(1, Ordering::Relaxed);
            linfo!(" DC{}{} подключен через Private Relay", dc, media_tag(is_media));
            bridge_ws(
                conn,
                ws,
                label,
        ...
        ws.close().await;
        lwarn!(" Private relay DC{}{}: initial write failed", dc, media_tag(is_media));
    }
